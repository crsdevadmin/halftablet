# HalfTablet — Full AWS Deployment (nothing runs on your desktop)
...
2. Settings: identifier `halftablet-db` · username `postgres` · set a strong password (save it)
3. Instance `db.t4g.micro`, storage 20 GB
4. Connectivity: **Public access: Yes** · new security group `halftablet-db-sg`
5. Additional configuration → **Initial database name: `halftablet`**
6. Create (~10 min) → open the DB → copy the **Endpoint**
7. EC2 → Security Groups → `halftablet-db-sg` → Inbound rules → add: PostgreSQL (5432), Source `0.0.0.0/0`
...
```
postgresql://postgres:YOUR_PASSWORD@YOUR-ENDPOINT.ap-south-1.rds.amazonaws.com:5432/halftablet?sslmode=require
```

## 3. Storage — S3 bucket

1. Console → **S3** → Create bucket: `halftablet-prescriptions-<something-unique>` · ap-south-1
2. Keep **Block all public access ON** (private medical documents)

## 4. Code — push to GitHub (the one desktop step)

Easiest: install **GitHub Desktop** (desktop.github.com), sign in, *Add local repository* → select the `halftablet` folder → Publish repository (private).

Alternatively, in a terminal: create an empty private repo on github.com, then
```bash
git remote add origin https://github.com/YOUR-USERNAME/halftablet.git
...
1. Console → **Amplify** → Create new app → **GitHub** → authorize → pick the `halftablet` repo, branch `main`
